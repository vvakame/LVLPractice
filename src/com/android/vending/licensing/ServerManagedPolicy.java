 ...
 */
final public class ServerManagedPolicy {
	...
	public ServerManagedPolicy(Context context, AESObfuscator obfuscator) {
		...
		mLastResponse = LicenseResponse.valueOf(mPreferences.getString(
				PREF_LAST_RESPONSE, LicenseResponse.RETRY.toString()));
		mValidityTimestamp = Long.parseLong(mPreferences.getString(
				PREF_VALIDITY_TIMESTAMP, DEFAULT_VALIDITY_TIMESTAMP));
		mRetryUntil = Long.parseLong(mPreferences.getString(PREF_RETRY_UNTIL,
				DEFAULT_RETRY_UNTIL));
		mMaxRetries = Long.parseLong(mPreferences.getString(PREF_MAX_RETRIES,
				DEFAULT_MAX_RETRIES));
		mRetryCount = Long.parseLong(mPreferences.getString(PREF_RETRY_COUNT,
				DEFAULT_RETRY_COUNT));
	}
